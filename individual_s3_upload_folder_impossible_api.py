# upload folders using ImpossibleCloud
import os
import time
...
# Initialize S3 client for ImpossibleCloud
s3 = boto3.client(
    service_name='s3',
...
def create_bucket_if_not_exists():
    ...
    try:
        ...
        if error_code == '404':
            try:
                # For ImpossibleCloud, region in CreateBucketConfiguration is ignored
                # Bucket is created in the endpoint region
...
def upload_files():
    ...
    for item_name, item_path in files_to_upload:
        ...
        try:
            # Upload without ACL parameter (ImpossibleCloud doesn't support ACLs)
            s3.upload_file(
                item_path,
                bucket_name,
                item_name,
...
if __name__ == "__main__":
    print("=" * 70)
    print("ImpossibleCloud Storage Uploader")
    print("=" * 70)
    print(f"Folder: {folder_path}")
    print(f"Bucket: {bucket_name}")
    print(f"Endpoint: https://eu-central-2.storage.impossibleapi.net")
    print("=" * 70 + "\n")
